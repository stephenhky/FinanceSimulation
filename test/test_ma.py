...
class TestMovingAverage(unittest.TestCase):
    def test_sp500(self):
        testdf = get_movingaverage_price_data('VOO', '2016-01-01', '2020-02-01', 200)
        assert testdf.iloc[0]['TimeStamp'].date().strftime('%Y-%m-%d') == '2016-01-04'

    def test_abnb(self):
        testdf = get_movingaverage_price_data('ABNB', '2016-01-01', '2020-02-01', 200)
        assert testdf.iloc[0]['TimeStamp'].date().strftime('%Y-%m-%d') == '2020-12-10'

    def test_average(self):
        testmadf = get_movingaverage_price_data('^GSPC', '2016-01-01', '2020-02-01', 200)
        testdf = get_yahoofinance_data('^GSPC', '2016-01-01', '2020-02-01')
        ...
        assert malastrow_data['TimeStamp'] == testdf['TimeStamp']
